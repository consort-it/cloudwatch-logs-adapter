...
public class AuthorizationFilter extends SecurityFilter {
  ...
  private static Config createSecurityConfig(final String authorizerName, final String roleName) {
    ...
    if (!StringUtils.isBlank(authorizerName) && !StringUtils.isBlank(roleName)) {
      config.addAuthorizer(authorizerName, new CloudWatchLogsAdapterAttributeAuthorizer(AUTHORIZER_SCOPE, ""));
    }
    config.setHttpActionAdapter(new RestHttpActionAdapter());
  ...
  public void handle(Request request, Response response) {
    if (!request.requestMethod().equals("OPTIONS") && !"1".equals(request.queryParams("nosec"))/* && EnvironmentContext.getInstance().getenv("PROD_ENABLED") == null*/) {
